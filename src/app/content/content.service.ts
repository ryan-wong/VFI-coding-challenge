...
export class ContentService {
  ...
  saveContent (text: string, annotation: AnnotationModel[], uid: string) : void {
    var cleanAnnotation = JSON.parse(JSON.stringify(annotation));
    var payload = {
      text: text,
      annotation: cleanAnnotation
    };
    firebase.database().ref('/content/' + uid).set(payload);
  }
  ...
  getContent (uid: string) : Promise<ContentModel>{
    return firebase.database().ref('/content/' +
      uid).once('value').then(function(snapshot) {
        return {
